...
course_info() {
    ...
    grep 'COURSE NAME: ' $file | grep -i '[A-Z]* [A-Z]*$' -o | xargs printf '%s %s has \n' > courses
    grep 'CREDITS: ' $file | grep '[0-9]*$' -o | xargs printf '%d\n' > credits
        
    #courses       CONTAINING ALL COURSES
    #->tempcourses CONTAINING CURRENT/USED COURSES
    #credits       CONTAINING ALL CREDITS TO COURSES
    #->tempcredits CONTAINING CURRENT/USED COURSES
    #tempdone      STRING CONTAINING CORRECT OUTPUT

    #Only need to do once
    head -1 courses > tempcourses
    head -1 credits > tempcredits

    #Contains all used courses in var
    tempc=$(tail -1 tempcourses) 
    tempcr=$(tail -1 tempcredits)
    tempdone="$tempc$tempcr credits"
        
    echo "$tempdone" > finished

    exitcount=$(grep '[A-Z]*' courses | wc -l)
    ...
    do
        ...
        #CHECKING THIS WHEN SERVER WORKS ADDED HEAD
        #Appending next course to the tail
        tail  -$exitcount courses | head -1 >> tempcourses
        tail  -$exitcount credits | head -1 >> tempcredits
